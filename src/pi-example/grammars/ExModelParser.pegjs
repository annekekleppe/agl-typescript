...
ExModel = "model" ws name:variable ws "{" ws 
	entities:(Entity)* ws 
	"model" ws "wide" ws "Methods:" ws 
	methods:MethodList2? ws 
	"}" ws 
	{ return creator.createExModel({name:name, entities:entities, methods:methods}); }

Entity = "Entity" ws name:variable ws baseEntity:("base" ws baseEntitySub:EntityPiElemRef { return baseEntitySub; })? ws "{" ws 
...
Method = "method" ws name:variable ws "(" ws parameters:ParameterList5? ws "):" ws declaredType:TypePiElemRef ws "{" ws 
...
AbsExpression = "abs(" ws expr:ExExpression ws ")" ws 
	{ return creator.createAbsExpression({expr:expr}); }

ParameterRef = parameter:ParameterPiElemRef ws "." ws appliedfeature:AppliedFeature ws 
...
MethodCallExpression = "CALL" ws methodDefinition:MethodPiElemRef ws "(" ws args:ExExpressionList6? ws ")" ws 
	{ return creator.createMethodCallExpression({methodDefinition:methodDefinition, args:args}); }

IfExpression = "if" ws "(" ws condition:ExExpression ws ")" ws "then" ws 
	whenTrue:ExExpression ws 
	"else" ws 
	whenFalse:ExExpression ws 
	"endif" ws 
